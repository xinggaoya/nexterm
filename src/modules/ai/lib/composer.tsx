...
export function AiComposerProvider({ children }: ProviderProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("nexterm:ai-attach-file", onAttach);
    return () => window.removeEventListener("nexterm:ai-attach-file", onAttach);
  ...
  const submit = () => {
    ...
    // the prompt to the NEXTERM.md scan template before sending.
    ...
    if (commandSource.startsWith("/") || commandSource.startsWith("#")) {
      ...
      if (outcome.kind === "send-prompt") {
        ...
        if (outcome.commandName) {
          commandMarker = `<nexterm-command name="${outcome.commandName}" />`;
        }
      }
    }
