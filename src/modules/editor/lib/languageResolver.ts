...
/**
 * 扩展名 → 加载器。每个 loader 都是动态 import,语言包只在打开对应
 * 文件时才进入 bundle。
 *
 * loader 可以返回现成的 Extension(lang-* 包)或原始 StreamParser
 * (legacy-modes)。resolveLanguage 会把后者包进 StreamLanguage 再返回。
 */
const languages: Record<string, LanguageDescriptor> = {
  ...
  // legacy-modes:loader 返回原始 StreamParser,在下方统一包裹。
  sh: {
    label: "Shell",
