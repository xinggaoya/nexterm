...
Identifiers: npm package `nexterm`, Rust crate `nexterm` (lib `nexterm_lib`), helper binary `nexterm-agent` (musl) and `nexterm-wsl-watcher`, Tauri product `Nexterm`, bundle id `app.xinggaoya.nexterm`. Current version: `0.2.1` (kept in sync between `package.json`, `src-tauri/Cargo.toml` and `src-tauri/tauri.conf.json`). Licensed under Apache-2.0.
